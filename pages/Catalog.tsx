...
export const Catalog: React.FC = () => {
    ...
    return (
        <div className="min-h-screen pt-20 pb-24 container mx-auto px-4 md:px-6">
            <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                <div className="flex flex-col gap-1">
                    ...
                    {searchParam ? (
                        <button onClick={clearSearch} className="text-sm text-pudava-secondary flex items-center gap-1 hover:text-white">
                            <X size={14}/> Clear Search
                        </button>
                    ) : (
                        <p className="text-gray-400 text-sm">Find your perfect style.</p>
                    )}
                </div>
                
                <div className="glass-panel p-1 rounded-full flex gap-1 overflow-x-auto max-w-full no-scrollbar">
                    {['All', 'Women', 'Men', 'Kids'].map((cat) => (
                        <button
                            key={cat}
                            onClick={() => handleFilterChange(cat)}
                            className={`px-4 py-1.5 rounded-full text-xs font-bold tracking-wider transition-all whitespace-nowrap uppercase ${
                                filter === cat 
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                    {filteredProducts.map((product) => (
                        <div key={product.id} className="group flex flex-col gap-3 cursor-pointer" onClick={() => navigate(`/product/${product.id}`)}>
                            <div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-white/5">
                                ...
                                {(user?.role === 'admin' || user?.role === 'manager') && (
                                    <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button className="p-2 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-white hover:text-black transition-colors" onClick={(e) => {e.stopPropagation(); alert('Edit clicked')}}>
                                            <Edit2 size={12} />
                                        </button>
                                    </div>
                                )}
